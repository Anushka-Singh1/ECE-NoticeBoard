...
export const AdminPage: React.FC = () => {
  const { currentUser, isAdmin } = useAuth();
  const { notices, loading, addNotice, updateNotice, deleteNotice } =
    useNotices();
  ...
  const handleUpdateNotice = async (formData: {
    ...
    try {
      ...
      setSuccess("Notice updated successfully!");
      setNoticeToEdit(null); // Reset the notice being edited
      setTimeout(() => setSuccess(null), 3000);
    } catch (err) {
      console.error("Error updating notice:", err);
      setError("Failed to update notice. Please try again.");
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="mb-8">
          <h1 className="text-3xl font-bold text-gray-900">Admin Dashboard</h1>
          <p className="mt-2 text-gray-600">
            Manage notices from this dashboard. Create new notices or edit
            existing ones.
          </p>
        </div>
        ...
        <div className="grid gap-8 md:grid-cols-12">
          ...
          <div className="md:col-span-7 lg:col-span-8">
            ...
            {loading ? (
              ...
              <Alert variant="info">
                <p>
                  No notices have been posted yet. Create your first notice!
                </p>
              </Alert>
            ) : (
              <div className="space-y-6">
